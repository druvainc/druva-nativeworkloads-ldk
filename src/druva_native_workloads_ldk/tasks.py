"""A module containing the langchain tool for Druva Native Workloads tasks"""
...
class DruvaNativeWorkloadsTasksInput(BaseModel):
    """Structure for tasks tool input"""
    organization_id: Optional[str] = Field(description="organization_id, can be left empty if unclear")

class DruvaNativeWorkloadsTasksTool(BaseTool):
    """A langchain tool to retrieve Druva Native Workloads Tasks"""
    name = "get_druva_native_workloads_tasks"
    description = (
        "useful for getting druva native workloads "
        "tasks information for a specific organization "
        "in JSON format"
    )
    args_schema: Type[BaseModel] = DruvaNativeWorkloadsTasksInput
