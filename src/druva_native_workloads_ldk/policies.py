"""A module containing the langchain tool for Druva Native Workloads policies"""
...
class DruvaNativeWorkloadsPoliciesInput(BaseModel):
    """Structure for policies tool input"""
    organization_id: Optional[str] = Field(description="organization_id, can be left empty if unclear")

class DruvaNativeWorkloadsPoliciesTool(BaseTool):
    """A langchain tool to retrieve Druva Native Workloads Policies"""
    name = "get_druva_native_workloads_policies"
    description = (
        "useful for getting druva native workloads "
        "policies information for a specific organization "
        "in JSON format"
    )
    args_schema: Type[BaseModel] = DruvaNativeWorkloadsPoliciesInput
